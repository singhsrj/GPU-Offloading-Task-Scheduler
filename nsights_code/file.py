            ...
            with nvtx.annotate(f"Profile_Size_{n}", color="red"):
                ...
                try:
                    cpu_time, cpu_std, cpu_result = TaskProfiler.benchmark_task(
                        cpu_func, n, 'CPU', warmup=1, runs=3
                    )
                ...
                try:
                    gpu_time, gpu_std, gpu_result = TaskProfiler.benchmark_task(
                        gpu_func, n, 'GPU', warmup=1, runs=3
                    )
...
def main():
    ...
    # Define size ranges for each task - OPTIMIZED for 1000+ rows with faster profiling
    # Vector sizes: focus on the decision boundary range with fine granularity (35 sizes)
    vector_sizes_log = [int(10**(3 + i*0.12)) for i in range(35)]  # 35 sizes from 1K to ~25M
    vector_sizes = list(set(vector_sizes_log))  # Remove duplicates
    vector_sizes.sort()
    
    # Matrix sizes: comprehensive range from 32 to 1536 (35 sizes)
    matrix_sizes = [32, 36, 40, 44, 48, 52, 56, 60, 64, 72, 80, 88, 96, 104, 112, 128, 
                    144, 160, 176, 192, 224, 256, 288, 320, 352, 384, 448, 512, 576, 
                    640, 768, 896, 1024, 1280, 1536]
    
    # Reduction sizes: similar to vectors (35 sizes)
    reduction_sizes_log = [int(10**(3 + i*0.12)) for i in range(35)]
    reduction_sizes = list(set(reduction_sizes_log))  # Remove duplicates
    reduction_sizes.sort()
